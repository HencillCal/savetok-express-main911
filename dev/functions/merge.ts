...
const networkHeaders = (referer?: string, userAgent = USER_AGENT): HeadersInit => ({
...
const createUrlSource = (fileUrl: string, referer?: string, userAgent?: string) =>
  new UrlSource(fileUrl, {
    ...
    parallelism: 2,
    getRetryDelay: (previousAttempts: number) => (previousAttempts >= 2 ? null : 0.35 * (previousAttempts + 1)),
  });
